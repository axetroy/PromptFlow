...
import { useThemeMode, usePanelPosition, usePromptSearch, usePromptKeyboardNav, getEffectiveThemeSync } from '../../hooks';
    ...
    >
      <div className="search-wrapper">
        <input
          ...
          type="text"
          placeholder={t('panel.searchPlaceholder', 'Search prompts...')}
          value={localSearch}
      ...
      <div ref={listRef} id="promptflow-list">
        {showRecentSection && recentPrompts.length > 0 && (
          <>
            <div className="section-header">{t('panel.recentlyUsed', 'Recently Used')}</div>
            {recentPrompts.map((prompt, index) => renderPromptItem(prompt, index, index, 'recent'))}
            <div className="section-header">{t('panel.allPrompts', 'All Prompts')}</div>
          </>
        )}
        ...
        {filteredPrompts.length > 0 ? (
          ...
          !showRecentSection && (
            <div className="empty-state">
              {localSearch ? t('panel.emptyFound', 'No prompts found') : t('panel.emptyAvailable', 'No prompts available')}
            </div>
          )
        )}
      </div>
      ...
      <div id="promptflow-footer">
        <div className="footer-hint">
          <span className="footer-key">↑↓</span> {t('panel.navigate', 'Navigate')}
          <span className="footer-key">Enter</span> {t('panel.select', 'Select')}
          <span className="footer-key">Esc</span> {t('panel.close', 'Close')}
        </div>
...
export function showPromptPanel(options: PromptPanelOptions): void {
  const theme = getEffectiveThemeSync();
