...
function updatePreview(): void {
  ...
  const previewEl = activeModal.querySelector('.vf-preview-content') as HTMLElement;
  if (previewEl) {
...
export function showVariableInput(options: VariableInputOptions): void {
  ...
  const variableInputsHtml = activeVariables.map(variable => `
    <div class="vf-variable-item" style="margin-bottom: 16px;">
      ...
      <input 
        ...
        value="${escapeHtml(activeValues[variable.name])}"
        placeholder="${variable.defaultValue ? escapeHtml(variable.defaultValue) : `Enter ${escapeHtml(variable.name)}...`}"
        style="width: 100%; padding: 10px 12px; font-size: 14px; border: 1px solid #d9d9d9; border-radius: 6px; outline: none; box-sizing: border-box; transition: border-color 0.2s;"
      />
    </div>
  `).join('');
  ...
  content.innerHTML = `
    ...
    <div class="vf-body" style="flex: 1; overflow: auto;">
      <div style="padding: 24px;">
        ...
        <div>
          ...
          <div class="vf-preview" style="background-color: #f8f8f8; border: 1px solid #e8e8e8; border-radius: 8px; padding: 16px; font-size: 14px; line-height: 1.6; color: #333; max-height: 200px; overflow: auto; white-space: pre-wrap; word-break: break-word;">
            <span class="vf-preview-content">${escapeHtml(interpolate(prompt.content, activeValues))}</span>
          </div>
        </div>
      </div>
    </div>
    <div class="vf-footer" style="padding: 16px 24px; border-top: 1px solid #e8e8e8; display: flex; justify-content: space-between; align-items: center; background: #fafafa;">
      <div style="font-size: 12px; color: #999;">
        Press <kbd style="background: #fff; border: 1px solid #d9d9d9; border-radius: 4px; padding: 2px 6px; font-family: monospace;">Ctrl</kbd> + <kbd style="background: #fff; border: 1px solid #d9d9d9; border-radius: 4px; padding: 2px 6px; font-family: monospace;">Enter</kbd> to confirm, <kbd style="background: #fff; border: 1px solid #d9d9d9; border-radius: 4px; padding: 2px 6px; font-family: monospace;">Esc</kbd> to cancel
      </div>
  ...
  // Focus first input
  const firstInput = content.querySelector('.vf-variable-input') as HTMLInputElement;
  if (firstInput) {
